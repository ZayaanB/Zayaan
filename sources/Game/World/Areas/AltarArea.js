...
export class AltarArea extends Area
{
    constructor(model)
    {
        ...
        this.color = uniform(color('#37b8ff'))
        ...
        // Pool: hide the demonic skull eyes
        const skullEyes = this.references.items.get('skullEyes')
        if(skullEyes && skullEyes[0].userData.object)
            this.game.objects.disable(skullEyes[0].userData.object)
    ...
    setBeam()
    {
        ...
        bottomMaterial.outputNode = Fn(() =>
        {
            ...
            // Circular pool surface
            dist.greaterThan(0.5).discard()

            // Animated water ripples
            const ripple = texture(
                this.game.noises.perlin,
                vec2(dist.mul(5).sub(this.game.ticker.elapsedScaledUniform.mul(0.3)), 0)
            ).r

            const gooColor = this.game.fog.strength.mix(vec3(0), this.game.fog.color) // Fog
            const emissiveColor = this.color.mul(this.emissive)

            // Bright center fading outwards, modulated by ripples
            const glow = dist.remapClamp(0, 0.5, 1, 0.15).mul(ripple.mul(0.6).add(0.6))
            const finalColor = mix(gooColor, emissiveColor, glow)
    ...
    setDeathZone()
    {
        ...
        zone.events.on(
            ...
            () =>
            {
                // Pool of Doom: splash, count the plunge, then reset the player
                this.animateBeam()
                this.animateBeamParticles()
                this.data.insert()
