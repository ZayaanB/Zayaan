...
export function DottedSurface({ className, ...props }: DottedSurfaceProps) {
	...
	useEffect(() => {
		...
		for (let ix = 0; ix < AMOUNTX; ix++) {
			for (let iy = 0; iy < AMOUNTY; iy++) {
                ...
                if (isOne) {
                    ...
                    colors1.push(0.0, 1.0, 0.533); // Neon Green
                    originalIndices1.push({ix, iy, index: p1Index * 3});
                    p1Index++;
                } else {
                    positions0.push(x, y, z);
                    colors0.push(0.0, 1.0, 0.533); // Neon Green
		...
		const handleResize = () => {
			...
			renderer.setSize(containerRef.current.clientWidth, containerRef.current.clientHeight);
			if (isLiteModeRef.current) {
				renderer.render(scene, camera); // Ensure it draws after resize
			}
		};
